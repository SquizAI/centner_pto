  ...
  const handlerResponse = await runWithRequestContext(requestContext, () => {
    return withActiveSpan(tracer, 'Next.js Server Handler', async (span) => {
      span?.setAttributes({
        ...
        monorepo: true,
        cwd: '/var/task/Centner_PTO/centner-pto-website',
      })
      if (!cachedHandler) {
        const { default: handler } = await import('/var/task/Centner_PTO/centner-pto-website/.netlify/dist/run/handlers/server.js')
