 ...
 * GET /api/social-media/facebook/posts?connectionId=xxx&limit=25
 * Fetches recent photos from a connected Facebook page
 */

import { NextRequest, NextResponse } from 'next/server';
import { z } from 'zod';
import { createClient } from '@/lib/supabase/server';
import { fetchFacebookPhotos } from '@/lib/social-media-api';
...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      ...
      // Mark which photos have been imported
      const photosWithImportStatus = photos.map((photo) => ({
        ...photo,
        isImported: importedPostIds.has(photo.id),
      }));
